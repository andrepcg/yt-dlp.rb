...
describe YtDlp::Video do
  before do
    @video = YtDlp::Video.new TEST_URL
  end
  ...
  describe '.download' do
    it 'should download videos without options' do
      YtDlp::Video.download TEST_URL
      assert_equal 1, Dir.glob(TEST_GLOB).length
    end

    it 'should download videos with options' do
      YtDlp::Video.download TEST_URL, output: TEST_FILENAME, format: TEST_FORMAT
      assert File.exist? TEST_FILENAME
    end

    it 'should return an instance of YtDlp::Video' do
      video = YtDlp::Video.download TEST_URL
      assert_instance_of YtDlp::Video, video
    end
  end

  describe '.get' do
    it 'should download videos, exactly like .download' do
      YtDlp::Video.get TEST_URL
  ...
  describe '#download' do
    ...
    it 'should raise ArgumentError if url is nil or empty' do
      assert_raises ArgumentError do
        YtDlp::Video.new(nil).download
      end

      assert_raises ArgumentError do
        YtDlp::Video.new('').download
      end
    end
  end
  ...
  describe '#filename' do
    ...
    # Broken due to how YtDlp::Video handles the filename
    ...
    # yt-dlp is at fault for not returning the correct filename.
