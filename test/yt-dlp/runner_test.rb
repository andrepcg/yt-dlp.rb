...
describe YtDlp::Runner do
  before do
    @runner = YtDlp::Runner.new(TEST_URL)
  end
  ...
  describe '#initialize' do
    it 'should take options as a hash yet still have configuration blocks work' do
      r = YtDlp::Runner.new(TEST_URL, { some_key: 'some value' })
  ...
  describe '#executable_path' do
    it 'should set executable path automatically' do
      assert_match 'yt-dlp', @runner.executable_path
    end

    it 'should not have a newline char in the executable_path' do
      assert_match(/yt-dlp\z/, @runner.executable_path)
    end
  end
  ...
  describe '#to_command' do
    ...
    it 'should handle true boolean values' do
      ...
      assert_match(/yt-dlp .*--truthy-value\s--|"http.*/, @runner.to_command)
    end

    it 'should handle false boolean values' do
      @runner.options.false_value = false

      assert_match(/yt-dlp .*--no-false-value\s--|"http.*/, @runner.to_command)
    end

    it 'should not have newline char in to_command' do
      assert_match(/yt-dlp\s/, @runner.to_command)
    end
  end
