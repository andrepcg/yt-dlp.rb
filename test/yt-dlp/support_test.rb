...
TestKlass = Class.new do
  ...
  def executable_path
    usable_executable_path_for 'yt-dlp'
  end
end

describe YtDlp::Support do
  before do
    @klass = TestKlass.new
  end
  ...
  describe '#usable_executable_path' do
    it 'should detect system executable' do
      vendor_bin = File.join(Dir.pwd, 'vendor', 'bin', 'yt-dlp')
      Dir.mktmpdir do |tmpdir|
        ...
        usable_path = @klass.usable_executable_path_for('yt-dlp')
        assert_match usable_path, "#{tmpdir}/yt-dlp"
    ...
    it 'should not have a newline char in the executable_path' do
      assert_match(/yt-dlp\z/, @klass.executable_path)
    end
  end
  ...
  describe '#terrapin_line' do
    ...
    it 'should default to yt-dlp' do
      line = @klass.terrapin_line(@klass.quoted(TEST_URL))
      assert_includes line.command, "yt-dlp \"#{TEST_URL}\""
    end
  end
